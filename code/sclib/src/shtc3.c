...
LOG_MODULE_REGISTER(shtc3, CONFIG_SCLIB_LOG_LEVEL);
...
int sc_shtc3_read(sc_shtc3_read_t *out) {
  ...
  // Wake the sensor up.
  RET_IF_ERR(write_cmd(SC_SHTC3_CMD_WAKEUP));
  k_msleep(1);

  // Request measurement.
  RET_IF_ERR(write_cmd(SC_SHTC3_CMD_MEASURE_TFIRST_NORMAL));
  ...
  RET_IF_ERR(write_cmd(SC_SHTC3_CMD_SLEEP));
